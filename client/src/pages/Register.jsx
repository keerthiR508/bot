...
const Register = () => {
  ...
  const [profileType, setProfileType] = useState('IT');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await register(name, email, password, role, profileType, company);
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 via-purple-900 to-black p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 w-full max-w-md shadow-2xl border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-indigo-200 text-sm font-medium mb-1">Full Name</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="block text-indigo-200 text-sm font-medium mb-1">Role</label>
            <select
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all [&>option]:bg-indigo-900"
              value={role}
          ...
          {role === 'Candidate' && (
            <div>
              <label className="block text-indigo-200 text-sm font-medium mb-1">Profile Type</label>
              <select
                className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all [&>option]:bg-indigo-900"
                value={profileType}
          ...
          {role === 'Recruiter' && (
            <div>
              <label className="block text-indigo-200 text-sm font-medium mb-1">Company Name</label>
              <input
                type="text"
                required
                className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white placeholder-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                placeholder="e.g. IBM, Infosys"
                value={company}
